...
if (mcpConfig && !mcpConfig.mcpServers?.['ai-huabu']) {
  failures.push('Missing MCP server entry: ai-huabu')
}

const marketplace = readJson('.agents/plugins/marketplace.json', repositoryRoot)
if (marketplace) {
  requireString(marketplace.name, 'marketplace.name')
  const entry = marketplace.plugins?.find((plugin) => plugin.name === 'ai-huabu-codex-plugin')
  if (!entry) {
    failures.push('Marketplace does not list ai-huabu-codex-plugin')
  } else {
    requireString(entry.source?.path, 'marketplace.plugins[].source.path')
    if (entry.source?.path !== './ai-huabu-codex-plugin') {
      failures.push('Marketplace source path must be ./ai-huabu-codex-plugin')
    }
...
if (failures.length) {
  console.error('AI Huabu plugin validation failed:')
  for (const failure of failures) console.error(`- ${failure}`)
  process.exit(1)
}

console.log('AI Huabu plugin validation passed.')
