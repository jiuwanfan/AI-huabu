# AI Huabu Installation Guide / AI Huabu 安装指南

This guide explains how to install AI Huabu as a Codex plugin from GitHub or from a local checkout.

这份文档说明如何从 GitHub 或本地目录安装 AI Huabu Codex 插件。
...
This guide uses the public repository `jiuwanfan/AI-huabu`.

本文档使用公开仓库 `jiuwanfan/AI-huabu`。
...
Use this path only when the published Git branch includes the built `ai-huabu-codex-plugin/packages/*/dist` runtime files. Codex plugin installation does not run `npm run setup` for users automatically.

只有当发布分支包含已经构建好的 `ai-huabu-codex-plugin/packages/*/dist` 运行时文件时，才建议使用这种方式。Codex 安装插件时不会自动替用户运行 `npm run setup`。
...
```bash
codex plugin marketplace upgrade ai-huabu
codex plugin add ai-huabu-codex-plugin@ai-huabu
```
...
```bash
codex plugin remove ai-huabu-codex-plugin
```

If you added the local or Git marketplace only for this plugin, you can also remove that marketplace from Codex:

如果你添加这个 marketplace 只是为了安装 AI Huabu，也可以从 Codex 中移除该 marketplace：

```bash
codex plugin marketplace remove ai-huabu
```
...
### Codex cannot find AI Huabu / Codex 找不到 AI Huabu

English:

1. Confirm the plugin was installed with `codex plugin add ai-huabu-codex-plugin@ai-huabu`.
2. Fully restart Codex or open a new chat.
3. Try `@AI Huabu 打开 AI Huabu。` again.

中文：

1. 确认已经执行 `codex plugin add ai-huabu-codex-plugin@ai-huabu`。
2. 完全重启 Codex，或新开一个对话。
3. 再输入 `@AI Huabu 打开 AI Huabu。`。
...
By default, canvas data is stored under `.ai-huabu/` in the active workspace. To override it:

中文：

默认情况下，画布数据保存在当前工作区的 `.ai-huabu/` 目录。可以通过环境变量改到其他位置：

```bash
export AI_HUABU_HOME="/path/to/ai-huabu-data"
```
