---
name: auto-ai-huabu-edit-mode
description: "Use when the user wants AI Huabu to automatically process edit jobs submitted from the canvas button, including 开启自动修图模式, 继续自动修图, 自动监听画布修改, 点按钮后自动修图, or continuous AI Huabu annotation edit mode."
---

# Auto AI Huabu Edit Mode
...
This workflow must use `watch_edit_requests`, `create_image_version`, `save_snapshot`, and `update_edit_request`. If these tools are not callable in the current Codex thread, stop and tell the user:

```text
AI Huabu 插件已经识别到，但自动修图工具没有加载出来。如果你刚安装或刚更新过插件，请完全退出并重新打开 Codex，再发送同一句需求；插件不需要重新安装。
```
...
2. Call `watch_edit_requests` with `waitMs` around 30000-45000 and `claim: true`.
...
5. Continue watching for another request when the user asked for continuous mode. Stop only when the user says to stop, the goal is paused, or there is a blocking clarification.
...
以后要继续时，回到 Codex 说：AI Huabu 继续自动修图。
```
...
- "收到一个修图任务，我开始处理。"
- "新版已经放到右侧，旧图保留。"
- "已停止监听。你可以继续查看或标注画布；以后要继续时，在 Codex 说“AI Huabu 继续自动修图”。"
