---
name: auto-ai-huabu-skill-mode
description: "Use when the user wants AI Huabu to process Skill jobs submitted from the canvas Skill panel, including 继续处理 Skill, 处理画布 Skill, 小红书封面 Skill, or canvas skill requests."
---

# Auto AI Huabu Skill Mode

This skill processes executable AI Huabu Skill requests submitted from the canvas Skill panel. Current executable scope covers `xiaohongshu-cover`, `youtube-thumbnail`, `cross-platform-adapt`, `product-marketing-set`, `logo-and-brand`, and `marketing-brochure`.
...
```text
AI Huabu 插件已经识别到，但 Skill 处理工具没有加载出来。如果你刚安装或刚更新过插件，请完全退出并重新打开 Codex，再发送同一句需求；插件不需要重新安装。
```
...
以后要继续时，回到 Codex 说：AI Huabu 继续处理 Skill。
```
