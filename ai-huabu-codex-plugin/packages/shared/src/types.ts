...
export type CanvasSkillCategory =
  | 'social_media'
  | 'e_commerce'
  | 'branding'
  | 'marketing'
...
export type CanvasSkillEntrypoint =
  | 'canvas_selection'
  | 'chat_skill_panel'
  | 'natural_language'

export type CanvasSkillCapability =
  | 'image_generation'
  | 'image_editing'
  | 'text_generation'
  | 'layout'
...
export type CanvasSkillRunStatus =
  | 'queued'
  | 'planning'
  | 'requires_external_generation'
  | 'running'
  | 'applying_actions'
  | 'completed'
  | 'failed'
...
export interface AiHuabuShapeMeta {
  aiHuabuRole?: AiHuabuRole
  aspectRatio?: string
